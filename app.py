            ...
            for col, idx in zip([card_col1, card_col2], [i, i + 1]):
                ...
                with col:
                    st.markdown(
                        f"""
                        <div class="em-card">
                            <div class="em-card-title">{row['Symbol']}</div>
                            <div class="em-card-sub">Preis: <span class="em-card-number">{row['Preis']:.2f}</span></div>
                            <div class="em-card-sub" style="color:{color};">
                                Change: <span class="em-card-number">{row['Change %']:.2f}%</span>
                            </div>
                            <div class="em-card-sub">RSI: <span class="em-card-number">{row['RSI']:.1f}</span></div>
                            <div class="em-card-sub">Trend: {row['Trend']}</div>
                            <div class="em-card-sub"><strong>{row['Kategorie']}</strong></div>
                        </div>
                        """,
                        unsafe_allow_html=True
                    )

                    # MINI-CHART DIREKT IN DER KARTE
                    try:
                        mini_chart_data = load_data(
                            row["Symbol"],
                            period="1mo",
                            interval="1d"
                        )
                        mini_chart_data = calculate_indicators(mini_chart_data)

                        st.markdown('<div class="em-mini-chart-wrap">', unsafe_allow_html=True)
                        ...
                        st.plotly_chart(
                            mini_fig,
                            width="stretch",
                            key=f"watchlist_mini_chart_{row['Symbol']}_{idx}"
                        )
                        st.markdown('</div>', unsafe_allow_html=True)
                    except Exception:
                        pass
