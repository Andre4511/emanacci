...
def render_compare_cards(compare_df: pd.DataFrame):
    if compare_df is None or compare_df.empty:
        st.info("Keine Vergleichsdaten vorhanden.")
        return

    for i in range(0, len(compare_df), 2):
        col1, col2 = st.columns(2)

        for col, idx in zip([col1, col2], [i, i + 1]):
            if idx >= len(compare_df):
                continue

            row = compare_df.iloc[idx]

            symbol = str(row.get("Symbol", "-"))
            price = float(row.get("Preis", row.get("Close", 0.0)))
            perf = float(row.get("Performance %", row.get("Change %", row.get("Performance", 0.0))))
            day_change = float(row.get("Tagesänderung %", row.get("Day Change %", row.get("Tagesänderung", 0.0))))
            ...
            with col:
                ...
                info_col, chart_col = st.columns([1.55, 1.0])
                ...
                with info_col:
                    st.markdown(
                        ...
                        <div class="em-card">
                            <div class="em-card-title">{symbol}</div>
                            <div class="em-card-sub">Preis: <span class="em-card-number">{price:.2f}</span></div>
                            <div class="em-card-sub" style="color:{perf_color};">
                                Performance: <span class="em-card-number">{perf:.2f}%</span>
                            </div>
                            <div class="em-card-sub">Tagesänderung: <span class="em-card-number">{day_change:.2f}%</span></div>
                            <div class="em-card-sub">Trend: {trend}</div>
                            <div class="em-card-sub">RSI: <span class="em-card-number">{rsi:.2f}</span></div>
                        </div>
                        """,
                        unsafe_allow_html=True
                    )
                ...
                with chart_col:
                    st.markdown('<div class="em-watch-card-right">', unsafe_allow_html=True)
                    st.markdown('<div class="em-watch-card-mini-title">1M Verlauf</div>', unsafe_allow_html=True)

                    try:
                        mini_data = load_data(symbol, period="1mo", interval="1d")
                        ...
                        st.plotly_chart(
                            mini_fig,
                            width="stretch",
                            key=f"compare_mini_chart_{symbol}_{idx}"
                        )
                    except Exception:
                        st.caption("Kein Mini-Chart verfügbar.")
